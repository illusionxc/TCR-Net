"""
Nature-style supplemental figures (v2): cleaner layout, richer chart styles.

Outputs (PDF only):
  fig_nature_supp1_confusion_triplet.pdf
  fig_nature_supp2_ablation_lollipop.pdf
  fig_nature_supp3_robustness_errorbar.pdf
  fig_nature_supp4_case_dualpanel.pdf
  fig_nature_supp5_significance_strip.pdf
  fig_nature_supp6_violin_risk.pdf
  fig_nature_supp7_scatter_tradeoff.pdf
  fig_nature_supp8_singlecol_1x2_a.pdf
  fig_nature_supp9_singlecol_1x2_b.pdf
"""
...
def fig1_confusion_triplet() -> None:
    ...
    # 4th panel: improvement of TCR-Net over Rule-Based for each class transition.
    ax = axes[3]
...
def fig2_ablation_lollipop() -> None:
    """Cleaner than crowded grouped bars: three aligned lollipop strips."""
...
def fig6_violin_risk() -> None:
    """
    Violin plot: 1×2 layout, single-column.
    (a) TCR-Net risk-score separability across classes.
    (b) Top-3 methods comparison on high-risk class only.
    """
    tab = load("tab_5_6_main_overall_performance.csv")
    ...
    # ── (a) TCR-Net risk-score separability ──
    ax = axes[0]
    ...
    # ── (b) Top-3 methods on high-risk class ──
    ax = axes[1]
    ...
    vp["cmeans"].set_color("#333333")
    ax.set_xticks([1, 2, 3])
    ax.set_xticklabels(["TCR-Net", "Unified", "Rule-Based"])
...
def fig7_scatter_tradeoff() -> None:
    """
    Scatter plot: performance-operation tradeoff.
    Problem connection:
      practical gateway needs high HRR with low FAR and strong Macro-F1.
    """
    tab = load("tab_5_6_main_overall_performance.csv")
    if tab.empty:
        return
    ...
    # (a) FAR vs HRR (point size by Macro-F1)
    ax = axes[0]
...
def fig8_singlecol_1x2_a() -> None:
    """Requested 1x2 single-column figure A: threshold sensitivity + scenario mean bars."""
    cross = load("fig_5_10_cross_scenario_data.csv")
    pert = load("fig_5_11_distribution_perturbation_data.csv")
    if cross.empty or pert.empty:
        return
    ...
    # (a) TCR threshold sensitivity on three subsets
    ax = axes[0]
...
def fig9_singlecol_1x2_b() -> None:
    """Requested 1x2 single-column figure B: class-wise profile + significance bars."""
    ...
    # (a) class-wise recall profile (top 4)
    ax = axes[0]
