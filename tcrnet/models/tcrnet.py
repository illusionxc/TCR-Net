...
class TCRNet(nn.Module):
    ...
    def __init__(self, config: Dict, meta: Dict[str, int]) -> None:
        ...
        self.use_cross_attention = bool(self.model_cfg.get("use_cross_attention", True))
        self.use_typed_rule_features = bool(
            self.model_cfg.get("use_typed_rule_features", False)
        )
        self.stabilize_aux_evidence = bool(
            self.model_cfg.get("stabilize_aux_evidence", False)
        )
        default_ca_mode = "bidirectional" if self.use_cross_attention else "none"
        self.cross_attention_mode = str(
            self.model_cfg.get("cross_attention_mode", default_ca_mode)
        ).lower()
        ca_aliases = {
            "both": "bidirectional",
            "event_to_history": "current_to_history",
            "history_to_event": "history_to_current",
            "off": "none",
        }
        self.cross_attention_mode = ca_aliases.get(
            self.cross_attention_mode, self.cross_attention_mode
        )
        valid_ca_modes = {
            "bidirectional", "current_to_history", "history_to_current", "none",
        }
        if self.cross_attention_mode not in valid_ca_modes:
            raise ValueError(
                "paper_model.cross_attention_mode must be "
                f"{sorted(valid_ca_modes)}, got={self.cross_attention_mode!r}"
            )
        ...
        ca_nhead = int(self.model_cfg.get("nhead", 4))
        ca_dropout = float(self.model_cfg.get("dropout", 0.1))
        self.ca_event_to_history = nn.MultiheadAttention(
            self.d_model, ca_nhead, dropout=ca_dropout, batch_first=True)
        self.ca_history_to_event = nn.MultiheadAttention(
            self.d_model, ca_nhead, dropout=ca_dropout, batch_first=True)
        ...
        typed_rule_dim = 5 if self.use_typed_rule_features else 0
        self.aux_head = nn.Sequential(
            nn.Linear(self.d_model * 2 + 3 + typed_rule_dim, self.d_model),
            nn.GELU(),
            nn.Dropout(float(self.model_cfg["dropout"])),
    ...
    def _shared_encode(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def _sequence_encode(self, history_seq: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:


        if not self.use_sequence:
            B = history_seq.shape[0]
            z = torch.zeros(B, self.d_model, device=history_seq.device, dtype=history_seq.dtype)
    ...
    def _mahalanobis(self, h_seq: torch.Tensor, intent_id: torch.Tensor) -> torch.Tensor:
        ...
        for cls in intent_id.unique():
            idx = int(cls.item())
            mask = intent_id == cls
            delta = h_seq[mask] - self.seq_mu_bank[idx]
            precision = _safe_inverse(self.seq_cov_bank[idx], self.cov_eps)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        h, z, enc_tokens = self._shared_encode(batch)
        intent = batch["intent_id"].long()


        h_seq, seq_tokens = self._sequence_encode(batch["history_seq"])
        ...
        ca_enabled = (
            ...
            and self.use_sequence
        )
        h_aug, h_seq_aug = h, h_seq
        if ca_enabled and self.cross_attention_mode in {
            "bidirectional", "current_to_history",
        }:

            h_ca = self.ca_event_to_history(
                h.unsqueeze(1), seq_tokens, seq_tokens
            )[0].squeeze(1)
            h_aug = h + h_ca

        if ca_enabled and self.cross_attention_mode in {
            "bidirectional", "history_to_current",
        }:

            h_seq_ca = self.ca_history_to_event(
                h_seq.unsqueeze(1), enc_tokens, enc_tokens
            )[0].squeeze(1)
            h_seq_aug = h_seq + h_seq_ca

        if ca_enabled:

            z_aug = self.z_proj(h_aug)
        else:
            z_aug = z


        p = self.prototype_bank[intent] if self.use_prototype else torch.zeros_like(z_aug)
        r = self._obs_vector(batch)
        r_hat = self.obs_predictor(torch.cat([z_aug, p], dim=-1))
        d_proto = (z_aug - p).pow(2).sum(dim=-1)
        ...
        r_seq = self._mahalanobis(h_seq_aug, intent)
        ...
        if self.decision_mode == "unified":
            ...
            evidence_scalars = torch.stack([r_cons, r_seq, r_rule], dim=-1)
            if self.stabilize_aux_evidence:


                evidence_scalars = torch.log1p(
                    torch.clamp(evidence_scalars, min=0.0)
                )
            decision_features = [
                h_aug, h_seq_aug, evidence_scalars,
            ]
            if self.use_typed_rule_features:


                size_feature = torch.clamp(
                    torch.log1p(torch.clamp(violations[:, 4], min=0.0))
                    / math.log1p(100.0),
                    min=0.0,
                    max=1.0,
                ).unsqueeze(-1)
                typed_rule = torch.cat([violations[:, :4], size_feature], dim=-1)
                decision_features.append(typed_rule)
            logits = self.aux_head(torch.cat(decision_features, dim=-1))
            unified_score = torch.zeros_like(r_cons)

        return {
            "h_shared": h_aug, "z": z_aug, "prototype": p,
            "r_obs": r, "r_hat": r_hat, "d_proto": d_proto, "d_attr": d_attr,
            "R_cons": r_cons, "h_seq": h_seq_aug, "R_seq": r_seq,
