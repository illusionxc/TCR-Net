...
SCENARIO_PROFILES = {
    "base": {
        ...
        "anomaly_probabilities": [0.12, 0.12, 0.12, 0.12, 0.17, 0.15, 0.20],
        "workload_scale": 1.0,
        "volume_scale": 1.0,
        "network_shift": 0.0,
        "pressure_shift": 0.0,
    ...
    "transmission_inspection": {
        "task_probabilities": [0.30, 0.14, 0.18, 0.08, 0.12, 0.18],
        "anomaly_probabilities": [0.11, 0.10, 0.13, 0.17, 0.13, 0.12, 0.24],
        "workload_scale": 1.10,
        "volume_scale": 1.15,
        "network_shift": -0.02,
        "pressure_shift": 0.05,
    ...
    "renewable_station": {
        "task_probabilities": [0.34, 0.10, 0.12, 0.08, 0.18, 0.18],
        "anomaly_probabilities": [0.09, 0.13, 0.09, 0.14, 0.19, 0.12, 0.24],
        "workload_scale": 1.25,
        "volume_scale": 1.30,
        "network_shift": -0.06,
        "pressure_shift": 0.10,
    ...
    "substation_maintenance": {
        "task_probabilities": [0.14, 0.20, 0.26, 0.14, 0.08, 0.18],
        "anomaly_probabilities": [0.09, 0.11, 0.13, 0.14, 0.15, 0.16, 0.22],
        "workload_scale": 0.95,
        "volume_scale": 0.90,
        "network_shift": 0.00,
        "pressure_shift": 0.14,
...
class SyntheticConfig:
    ...
    num_modes: int
    scenario: str = "base"
    history_crop_length: int | None = None
    ...
    def from_dict(cls, cfg: Dict) -> "SyntheticConfig":
        ...
        merged.setdefault("scenario", "base")
        merged.setdefault("history_crop_length", None)
        return cls(**merged)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key, value in sample.items():
            if isinstance(value, np.ndarray):
                ...
                raise TypeError(f"Unsupported key type: key={key}, type={type(value)}")
        return result
...
def _generate_file_properties(
    ...
    profile_shift = SCENARIO_PROFILES.get(scenario)
    if profile_shift is None:
        raise ValueError(
            f"Unknown synthetic scenario {scenario!r}, choices: {sorted(SCENARIO_PROFILES)}"
        )
    workload = np.clip(workload * float(profile_shift["workload_scale"]), 0.0, 1.0)
    file_volume = np.clip(file_volume * float(profile_shift["volume_scale"]), 0.0, 1.0)
    network_quality = np.clip(
        network_quality + float(profile_shift["network_shift"]), 0.5, 1.0
    )
    time_pressure = np.clip(
        time_pressure + float(profile_shift["pressure_shift"]), 0.0, 1.0
    )
...
def _generate_event(
    ...
    scenario_profile = SCENARIO_PROFILES.get(cfg.scenario)
    if scenario_profile is None:
        raise ValueError(
            f"Unknown synthetic scenario {cfg.scenario!r}, choices: {sorted(SCENARIO_PROFILES)}"
        )
    task_probabilities = scenario_profile["task_probabilities"]
    if task_probabilities is None:
        task_id = int(rng.integers(0, cfg.num_intents))
    else:
        if len(task_probabilities) != cfg.num_intents:
            raise ValueError(
                f"Scenario {cfg.scenario!r} has {len(task_probabilities)} task probabilities, "
                f"but num_intents={cfg.num_intents}"
            )
        task_id = int(rng.choice(cfg.num_intents, p=task_probabilities))
    ...
    initial_state, context, file_params, ext_id, hour = \
        _generate_file_properties(rng, task_id, domain_shift, cfg.scenario)
    ...
    if allow_anomaly and rng.random() < cfg.anomaly_ratio:
        anomaly_label = 1
        type_weights = scenario_profile["anomaly_probabilities"]
        if len(type_weights) != len(ANOMALY_NAMES) - 1:
            raise ValueError(
                f"Scenario {cfg.scenario!r} must define an anomaly probability vector with length "
                f"{len(ANOMALY_NAMES) - 1}"
            )

        anomaly_type = int(
            rng.choice(np.arange(1, len(ANOMALY_NAMES)), p=type_weights)
        )
...
def build_dataloaders(
    ...
    if cfg.source in {"station_jsonl", "mock_station_jsonl"}:
        if cfg.data_dir is None:
            raise ValueError("data.data_dir must be specified")
    ...
    crop_length = cfg.history_crop_length
    if crop_length is not None:
        crop_length = int(crop_length)
        available = min(
            int(np.asarray(sample["history_seq"]).shape[0])
            for sample in train_s + val_s + test_s
        )
        if crop_length <= 0 or crop_length > available:
            raise ValueError(
                f"data.history_crop_length={crop_length} is outside the valid range [1, {available}]"
            )
        for sample in train_s + val_s + test_s:
            sample["history_seq"] = np.asarray(
                sample["history_seq"][-crop_length:], dtype=np.float32
            )
        meta["history_length"] = crop_length
    ...
    return (
        DataLoader(
            FileEventDataset(train_s), shuffle=True,
