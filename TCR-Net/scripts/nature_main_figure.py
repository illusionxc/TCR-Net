...
METHOD_COLORS_MAP = dict(zip(METHOD_ORDER, [
    C_TEAL,          # TCR-Net
    '#4A8BB7',       # Unified-Multimodal
    '#7A7A7A',       # w/o Rule
    '#A0A0A0',       # Sequence+Semantic
    '#B8B8B8',       # Content+Semantic
    '#C8C8C8',       # Rule-Based
    '#D8D8D8',       # Content-Only
]))
...
def panel_b(ax):
    ...
    #  F1 = (Low-F1 + Mid-F1 + High-F1) / 3 Precision/Recall 
    ...
    for risk_i in range(3):
        vals = [low_f1, mid_f1, high_f1][risk_i]  # shape (4,): 
        off = (risk_i - 1) * w
