...
def _compute_loss(
    ...
    l_proto = outputs["d_proto"][normal_mask].mean() if (model.use_prototype and normal_mask.any()) else torch.tensor(0.0, device=y3.device)
...
def _select_decision_calibration(
    ...
    mgrid = [
        float(value)
        for value in config.get("paper_score", {}).get(
            "hybrid_mix_grid", [0.5]
        )
    ]
    if not mgrid or any(value < 0.0 or value > 1.0 for value in mgrid):
        raise ValueError("paper_score.hybrid_mix_grid must be a nonempty list within [0, 1]")
...
def train(config: Dict, external_loaders=None) -> Dict:
    ...
    if external_loaders is None:
        train_l, val_l, test_l, meta = build_dataloaders(
            config, int(config["seed"])
        )
    else:
